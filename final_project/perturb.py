...
--------
    - time-evolution of perturbation
        --> entirely independent of what's happening on the grid.
            I.e., need not output
...
class PerturbedNetwork:
    ...
    def __post_init__(self):  # what should i do w this? generate tests?
        ...
        if not self.radius:
            self.radius = self.m / self.L_x * 0.01

        assert (
            len(self.source_center) == 2
        ), "Source point must be coordinate pair"
        assert self.m > 10, "Grid must be AT LEAST 10x10"
        assert (
            self.radius < 0.1 * self.m
        ), "Disturbance radius cannot span >10% of grid"
    ...
    def construct_A(self, m, delta_x):
        ...
        I_diag = sparse.eye(m)
        A = sparse.kron(I_diag, T) + sparse.kron(S, I_diag)
        A /= delta_x**2
    ...
    def construct_source(
        ...
        if f_type.lower() == "oscillatory":
            ...
            # 	sp = (sp[0]-1, sp[1]-1)
            circle = np.sqrt(
                (X_source - source_center[1] * self.L_x) ** 2
